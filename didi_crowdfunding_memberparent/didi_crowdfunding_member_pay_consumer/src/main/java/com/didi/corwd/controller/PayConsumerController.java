package com.didi.corwd.controller;
...
import com.didi.crowd.entrty.VO.OrderSuccessVO;
...
public class PayConsumerController {
    ...
    private Logger logger = LoggerFactory.getLogger(PayConsumerController.class);
    @ResponseBody
...
//            valueStr = new String(valueStr.getBytes("ISO-8859-1"), "utf-8");
        ...
        logger.warn(signVerified+"1111111111111");
        //——请在这里编写您的程序（以下代码仅作参考）——
...
//            valueStr = new String(valueStr.getBytes("ISO-8859-1"), "utf-8");
        ...
        boolean signVerified = AlipaySignature.rsaCheckV1(
                ...
                payProperties.getCharset(),
                payProperties.getSignType()); //调用SDK验证签名
        logger.warn("signVerified"+signVerified+"1111111111111");
        // ——请在这里编写您的程序（以下代码仅作参考）——
        if(true) {
            // 商户订单号
            ...
            BeanUtils.copyProperties(orderVO,orderSuccessVO);
            session.setAttribute("orderSuccessVO",orderSuccessVO);
            return "redirect:http://localhost:80/pay/do/success/order";
        }else {
            // 页面显示信息：验签失败
            return "redirect:http://localhost:80/pay/do/success/order";
